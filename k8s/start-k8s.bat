@echo off
rem Garante que o script sempre rode a partir da pasta raiz (SafeRoute), independente de onde o terminal esteja aberto
cd /d "%~dp0.."
...
echo Iniciando a Arquitetura SafeRoute K8s...
kubectl get hpa
kubectl apply -f ./k8s/1-postgres-db.yaml
kubectl apply -f ./k8s/4-config-server.yaml
echo Esperando 20 segundos para o Config Server estabilizar...
timeout /t 20 /nobreak >nul
kubectl apply -f ./k8s/5-eureka-server.yaml
echo Esperando 20 segundos para o Eureka ligar...
timeout /t 20 /nobreak >nul
...
echo Cluster iniciado! Verifique o pod no OpenLens.